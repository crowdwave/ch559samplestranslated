...
* Description:
 ...
 Support simple USB printer operation without processing USB printer commands
************************************************** *****************************/
#include <CH559.H>
#include <stdio.h>
#include <string.h>
...
#define FOR_ROOT_UDISK_ONLY 1 // Only used for U disk file operation of DP/DM port (using subroutine library CH559UFI/X), does not support U disk operation under HUB
...
//..... struct _Endp_Attr Endp_Attr[4]; //The attributes of the endpoint, supports up to 4 endpoints
    ...
    UINT8 GpVar1; // General variable 1
} xdata RootHubDev[2];
...
// pBuf += UH_TX_LEN;
                if(pBuf[1] == 0x09) //HID command processing
                {
        ...
        if (s == ERR_SUCCESS)
        {
            ...
            if (s == ERR_SUCCESS)
            {
                ...
                if (s == ERR_SUCCESS)
                {
                    ...
                    else if (dv_cls == 0x00 && if_cls == USB_DEV_CLASS_PRINTER && ((PXUSB_CFG_DESCR_LONG)TxBuffer) -> itf_descr.bInterfaceSubClass == 0x01 )// is a printer device
                    {
                        printf( "USB-Print OK\n" );
                        if((TxBuffer[19] == 5)&&(TxBuffer[20]&&0x80))
                        {
                            RootHubDev[RootHubIndex].GpVar = TxBuffer[20];//IN endpoint
                        }
                        else if((TxBuffer[19] == 5)&&((TxBuffer[20]&&0x80) == 0))
                        {
                            RootHubDev[RootHubIndex].GpVar1 = TxBuffer[20];//OUT endpoint
                        }
                        if((TxBuffer[26] == 5)&&(TxBuffer[20]&&0x80))
                        {
                            RootHubDev[RootHubIndex].GpVar = TxBuffer[27];//IN endpoint
                        }
                        else if((TxBuffer[26] == 5)&&((TxBuffer[20]&&0x80) == 0))
                        {
                            RootHubDev[RootHubIndex].GpVar1 = TxBuffer[27];//OUT endpoint
                        }
// RootHubDev[RootHubIndex].GpVar = ((PUSB_CFG_DESCR_LONG)TxBuffer) -> endp_descr[0].bEndpointAddress;// Save bulk transfer endpoint
                        s = CtrlSetUsbConfig( cfg ); // Set USB device configuration
                        if (s == ERR_SUCCESS)
                        {
...
// if(s == ERR_SUCCESS){
                            ...
                            s = CtrlGetXPrinterReport1( ); //Printer command
                            if(s == ERR_SUCCESS)
                            {
                                RootHubDev[RootHubIndex].DeviceStatus = ROOT_DEV_SUCCESS;
...
//}
                    ...
                    else if (dv_cls == 0x00 && if_cls == USB_DEV_CLASS_HID && ((PXUSB_CFG_DESCR_LONG)TxBuffer) -> itf_descr.bInterfaceSubClass <= 0x01 )// It is a HID device, keyboard/mouse, etc.
                    {
                        ...
                        RootHubDev[RootHubIndex].GpVar = s & USB_ENDP_ADDR_MASK ;// Save the address of the interrupt endpoint, bit 7 is used to synchronize the flag bit, cleared to 0
                    ...
                    {
                        s = AnalyzeHidIntEndp( TxBuffer ); // Analyze the address of the HID interrupt endpoint from the descriptor
                        RootHubDev[RootHubIndex].GpVar1 = s & USB_ENDP_ADDR_MASK ;// Save the address of the interrupt endpoint, bit 7 is used to synchronize the flag bit, cleared to 0
                        printf( "GetHubDescr: ");
                        s = CtrlGetHubDescr( );
                        if (s == ERR_SUCCESS)
                        {
                            ...
                            printf("\n");
                            RootHubDev[RootHubIndex].GpVar = ((PXUSB_HUB_DESCR)TxBuffer) -> bNbrPorts;// Save the number of HUB ports
                            if (RootHubDev[RootHubIndex].GpVar> HUB_MAX_PORTS)
                            {
                                RootHubDev[RootHubIndex].GpVar = HUB_MAX_PORTS;// Because when defining the structure DevOnHubPort, it is artificially assumed that each HUB does not exceed HUB_MAX_PORTS ports
                            }
                            ...
                            if (s == ERR_SUCCESS)
                            {
                                ...
                                for (i = 1; i <= RootHubDev[RootHubIndex].GpVar; i ++ )// Power on all ports of the HUB
                                {
...
// for (i = 1; i <= RootHubDev[RootHubIndex].GpVar; i ++) // Clear the connection status of each port of the HUB
// {
...
//}
//}
                                for (i = 1; i <= RootHubDev[RootHubIndex].GpVar; i ++) // Query the connection status of each HUB port
                                {
...
// if ((TxBuffer[0] != 0) && (DevOnHubPort[RootHubIndex][i-1].DeviceStatus != ROOT_DEV_SUCCESS) ){ //There is a device connection on the downstream port of HUB
// s = HubSetPortFeature( i, HUB_PORT_RESET ); // Reset the port with device connection
// if (s != ERR_SUCCESS) continue;
// // return s; // may be that the HUB is disconnected
// do // Query the reset port, until the reset is completed, display the status after completion
// {
// mDelaymS( 1 );
// s = HubGetPortStatus( i );
// if (s != ERR_SUCCESS) continue;//return s; // It may be that the HUB is disconnected
...
UINT8 EnumHubPort( UINT8 RootHubIndex)
{
    UINT8 i, s;
    printf( "EnumHubPort\n" );
    for (i = 1; i <= RootHubDev[RootHubIndex].GpVar; i ++) // Query whether the port of the hub has changed
    {
...
// for (i = 1; i <= RootHubDev[RootHubIndex].GpVar; i ++ ){ // Initialize each port of HUB
...
UINT16 SearchTypeDevice( UINT8 type)
{
    ...
    for (RootHubIndex = 0; RootHubIndex != 2; RootHubIndex ++) // The current search can avoid the problem that the device is unplugged in the middle and some information is not updated in time
    {
        ...
        {
            for (HubPortIndex = 1; HubPortIndex <= RootHubDev[RootHubIndex].GpVar; HubPortIndex ++ )// Search each port of external HUB
            {
...
* Input: PUINT8 buf lighting key value
* Output: None
* Return: None
************************************************** *****************************/
...
main()
{
    ...
    UINT8 buf[100]; //The length can be specified according to the application
    HUBFlag = 0;
    ...
    while (1)
    {
        ...
        if (loc != 0xFFFF) // Found it, what if there are two MOUSEs?
        {
            ...
            endp = len? DevOnHubPort[i][len-1].GpVar: RootHubDev[i].GpVar; // interrupt endpoint address, bit 7 is used for synchronization flag
...
// mDelaymS(1);
        ...
        // Use timing to simulate subjective needs, you need to operate the keyboard
        loc = SearchTypeDevice( DEV_TYPE_KEYBOARD ); // Search for the port number where the specified type of device is located on each port of ROOT-HUB and external HUB
        if (loc != 0xFFFF) // Found it, what if there are two KeyBoards?
        {
        ...
        for(i=0; i<2; i++)
        {
            ...
            {
                SelectHubPort( i, 0 ); // Select the ROOT-HUB port specified by the operation, set the current USB speed and the USB address of the operated device
                endp = RootHubDev[i].GpVar1; // The address of the interrupt endpoint, bit 7 is used for the synchronization flag
                if (endp & USB_ENDP_ADDR_MASK) // Endpoint is valid
                {
                    s = USBHostTransact( USB_PID_IN << 4 | endp & 0x7F, endp & 0x80? bUH_R_TOG | bUH_T_TOG: 0, 0 ); // CH559 transfers transactions, gets data, NAK does not retry
                    if (s == ERR_SUCCESS)
                    {
                        endp ^= 0x80; // Synchronization flag flip
                        RootHubDev[i].GpVar1 = endp; // Save the synchronization flag
                        len = USB_RX_LEN; // The received data length
                        if (len)
                        {
                            EnumHubPort(i);
        ...
        //Operate USB printer
        if((P4_IN&0x01) == 0) //P40 is low, start printing
        {
            ...
            if (loc != 0xFFFF) // Found it, what if there are two printers?
            {
                printf( "Query Printer @%04X\n", loc );
                i = (UINT8)( loc >> 8 );
                len = (UINT8)loc;
                SelectHubPort( i, len ); // Select the ROOT-HUB port specified by the operation, set the current USB speed and the USB address of the operated device
                endp = len? DevOnHubPort[i][len-1].GpVar: RootHubDev[i].GpVar; // Endpoint address, bit 7 is used for synchronization flag
                printf("%02X ",endp);
                if (endp & USB_ENDP_ADDR_MASK) // Endpoint is valid
                {
                    UH_TX_LEN = 64; // There is no data by default, so the status stage is IN
                    s = USBHostTransact( USB_PID_OUT << 4 | endp & 0x7F, endp & 0x80? bUH_R_TOG | bUH_T_TOG: 0, 0xffff );// CH559 transfer transaction, get data, NAK retry
                    if (s == ERR_SUCCESS)
                    {
                        endp ^= 0x80; // Synchronization flag flip
                        memset(TxBuffer,0,sizeof(TxBuffer));
        ...
        if (loc != 0xFFFF) // found
        {
            ...
            if (CH559DiskStatus >= DISK_MOUNTED) //U disk is ready
            {
                ...
                {
                    printf( "%02x File not found\n",(UINT16)CH559DiskStatus );
                    TotalCount = 100; //Ready to read the total length
                    printf( "The first %d characters read from the file are:\n",TotalCount );
                    while (TotalCount) //If the file is large and cannot be read at one time, you can call CH559ByteRead to continue reading, and the file pointer will automatically move backward
                    {
                        if (TotalCount> (MAX_PATH_LEN-1))
                        {
                            c = MAX_PATH_LEN-1; //There are many remaining data, and the length of a single read and write cannot exceed sizeof( mCmdParam.Other.mBuffer)
                        }
                        else
                        {
                            c = TotalCount; //The last remaining number of bytes
                        }
                        mCmdParam.ByteRead.mByteCount = c; //Request to read dozens of bytes of data
                        mCmdParam.ByteRead.mByteBuffer= &buf[0];
                        s = CH559ByteRead( ); //Read the data block in bytes, the length of a single read and write cannot exceed MAX_BYTE_IO, and the second call is followed by the previous read
                        TotalCount -= mCmdParam.ByteRead.mByteCount;//Count, minus the current number of characters actually read
                        for (i=0; i!=mCmdParam.ByteRead.mByteCount; i++)
                        {
                            printf( "%C", mCmdParam.ByteRead.mByteBuffer[i] );//Display the read character
                        }
                        if (mCmdParam.ByteRead.mByteCount <c )//The actual number of characters read is less than the number of characters required to read, indicating that the end of the file has been reached
                        {
                            printf( "\n" );
                ...
                /* If you want to start reading and writing from the specified position, you can move the file pointer
                        mCmdParam.ByteLocate.mByteOffset = 608; //Skip the first 608 bytes of the file to start reading and writing
                        CH559ByteLocate( );
                        mCmdParam.ByteRead.mByteCount = 5; //Read 5 bytes
                        mCmdParam.ByteRead.mByteBuffer= &buf[0];
                        CH559ByteRead( ); //Read the data from the 608th byte to the 612th byte of the file directly, the first 608 bytes are skipped
                        ...
                        CH559ByteWrite( ); //Add data at the end of the original file, and the newly added 13 bytes are placed at the end of the original file
                        mCmdParam.ByteWrite.mByteCount = 2; //Write 2 bytes of data
                        CH559ByteWrite( ); //Continue to add data after the original file
                        ...
                        CH559ByteWrite( ); //Write 0 bytes of data to automatically update the length of the file, so the file length is increased by 15. If you don't do this, then the file length will be automatically updated when CH559FileClose is executed
                */
                printf( "Create File\n" );
                strcpy( mCmdParam.Create.mPathName, "/NEWFILE.TXT" );//New file name, in the root directory, Chinese file name
                s = CH559FileCreate( ); //Create a new file and open it, if the file already exists, delete it and then create it
                mStopIfError( s );
                printf( "ByteWrite\n" );
                //In fact, it should be judged whether the length of the written data is consistent with the length of the defined buffer. If it is greater than the length of the buffer, it needs to be written multiple times
                i = sprintf( buf,"Note: \xd\xa This program reads and writes files on the USB flash drive in bytes, 559 simple demonstration functions.\xd\xa");/*Demo */
                for(c=0; c<10; c++)
                {
                ...
                mCmdParam.Modify.mFileTime = 0xffff; //Input parameter: new file time, if it is 0FFFFH, do not modify it, use the default time generated by the newly created file
                mCmdParam.Modify.mFileDate = MAKE_FILE_DATE( 2020, 8, 15 ); //Input parameters: new file date: 2016.01.15
                mCmdParam.Modify.mFileSize = 0xffffffff; //Input parameters: the new file length, the file is written in bytes, the length should be automatically updated when the file is closed by the library, so it is not modified here
                ...
                /*
                                                        ...
                                                        strcpy( mCmdParam.Open.mPathName, "/111.TXT" ); //File name, the file is in the C51 subdirectory
                                                        s = CH559FileOpen( ); //Open the file
                                                        if(s == ERR_SUCCESS){
                ...
                */
                ...
                i = CH559FileErase( ); //Delete the file and close
                if (i != ERR_SUCCESS)
                {
...
while(1); //The code stops here during U disk demonstration
        }
    }
}
