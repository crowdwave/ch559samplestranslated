...
* Description        :
 ...
 演示查找/C51/CH559HFT.C文件，如果找到则字节读出文件前100字节内容；
 演示创建/NEWFILE.TXT文件，循环字节写入指定字符串，并修改文件属性；
 演示修改1.TXT文件名为2.C
 演示删除/OLD.TXT文件；
...
UINT16	SearchTypeDevice( UINT8 type );              //搜索指定设备 
...
UINT8 RenameFilename(PUINT8 FilenameBuf,UINT8 len)
{
    ...
    PX_FAT_DIR_INFO  mFileDir;	
		i = CH559FileQuery();//查询文件信息，以便将文件信息调入内存再修改
		if( i == ERR_SUCCESS ){
			mFileDir = (PX_FAT_DIR_INFO)(pDISK_BASE_BUF + CH559vFdtOffset);//内存中，当前FDT的起始地址
      ...
      while(i<len){
		...
		if( i == ERR_SUCCESS ){
			mCmdParam.Close.mUpdateLen = 0;             //不更新文件长度
			i = CH559FileClose( );          					
		}  				
    return i;
}
