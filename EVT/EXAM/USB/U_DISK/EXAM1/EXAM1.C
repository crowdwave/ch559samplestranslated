...
* Description:
 ...
 Demonstrate searching for the /C51/CH559HFT.C file, if found, read the first 100 bytes of the file;
 Demonstrate the creation of the /NEWFILE.TXT file, write cyclic bytes into the specified string, and modify the file attributes;
 Demo to modify the 1.TXT file name to 2.C
...
UINT16 SearchTypeDevice( UINT8 type ); //Search the specified device
...
* Description: File name rewriting
* Input:
                   PUINT8 FilenameBuf file name without path information
* Output: None
* Return: None
************************************************** *****************************/
UINT8 RenameFilename(PUINT8 FilenameBuf,UINT8 len)
{
    UINT8 i;
    PX_FAT_DIR_INFO mFileDir;
i = CH559FileQuery();//Query file information, so that the file information can be transferred to the memory and then modified
if( i == ERR_SUCCESS ){
mFileDir = (PX_FAT_DIR_INFO)(pDISK_BASE_BUF + CH559vFdtOffset);//In the memory, the starting address of the current FDT
      memset(mFileDir->DIR_Name,0x20,11);
      i = 0;
      while(i<len){
        if(FilenameBuf[i] =='.'){
          i++;
          break;//File name suffix
        }
        mFileDir->DIR_Name[i] = FilenameBuf[i];
        i++;
      }
memcpy( &mFileDir->DIR_Name[8], &FilenameBuf[i],len-i );
}
mCmdParam.Modify.mFileAttr = mFileDir->DIR_Attr;
...
if( i == ERR_SUCCESS ){
mCmdParam.Close.mUpdateLen = 0; //Do not update the file length
i = CH559FileClose( );
}
    return i;
}
...
{
    UINT8 s, c,i;
    UINT16 TotalCount,loc;
    UINT8 buf[100]; //The length can be specified according to the application
    UINT8 Rename[6];
    Rename[0]= '2';Rename[1]= '2';Rename[2]= '2';Rename[3]= '2';Rename[4]='.';Rename[5]= 'C';
...
if (mCmdParam.ByteRead.mByteCount <c )//The number of characters actually read is less than the number of characters required to read, indicating that the end of the file has been reached
{
printf( "\n" );
...
CH559ByteWrite( ); //Write 0 bytes of data to automatically update the length of the file, so the file length is increased by 15. If you don't do this, the file length will be automatically updated when CH559FileClose
*/
printf( "Create File\n" );
...
mStopIfError( s );
printf( "ByteWrite\n" );
//Actually, it should be judged whether the length of the written data matches the length of the defined buffer. If it is greater than the length of the buffer, multiple writes are required
i = sprintf( buf,"Note: \xd\xa This program reads and writes U disk files in bytes, 559 simple demonstration functions. \xd\xa");/*Demo */
for(c=0; c<10; c++)
{
...
printf( "Modify\n" );
mCmdParam.Modify.mFileAttr = 0xff; //Input parameter: new file attribute, if it is 0FFH, it will not be modified
