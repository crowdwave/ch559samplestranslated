...
* Description    : 将程序空间的字符串复制到内部RAM中,返回字符串长度
* Input          : UINT8 idata *iDestination, 目标字符串
                   UINT8 code *iSource 源字符串
* Output         : None
* Return         : UINT8    i 返回字符个数
*******************************************************************************/
...
UINT8   CreateDirectory( void )
{
    ...
    else
    {
        ...
        UpDirCluster = CH559vStartCluster;                             //保存上级目录的起始簇号
    }
    if ( i == ERR_SUCCESS )                                            //成功获取上级目录的起始簇号
    {
        i = CH559FileOpen( );                                          //打开本级子目录
        if ( i == ERR_SUCCESS )
        {
        ...
        {
            i = CH559FileCreate( );                                    //以创建文件的方法创建目录
            if ( i == ERR_SUCCESS )
            {
                if ( pDISK_FAT_BUF == pDISK_BASE_BUF )
                {
                    memset(pDISK_FAT_BUF,0,CH559vSectorSizeH*256);     //如果FILE_DATA_BUF与DISK_BASE_BUF合用则必须清除磁盘缓冲区
                }
                ...
                *(pDISK_FAT_BUF+0x1A) = ( (PUINT8I)&CH559vStartCluster )[3];//自身的起始簇号
                ...
                *(pDISK_FAT_BUF+0x20+0x1A) = ( (PUINT8I)&UpDirCluster )[3];//上级目录的起始簇号
...
//              for ( count = 0x40; count != CH559vSectorSizeH*256; count ++ ) {  /* 清空目录区剩余部分 */
//                  *DirXramBuf = 0;
//                  DirXramBuf ++;
//              }
                ...
                if ( i == ERR_SUCCESS )
                {
                    ...
                    for ( j = 1; j != CH559vSecPerClus; j ++ )
                    {
                        ...
                        mCmdParam.Write.mSectorCount = 1;
                        mCmdParam.Write.mDataBuffer = pDISK_FAT_BUF;         //指向文件数据缓冲区的起始地址
                        i = CH559FileWrite( );                               //清空目录的剩余扇区
                        if ( i != ERR_SUCCESS )
                        {
                            break;
                        }
                    }
                    if ( j == CH559vSecPerClus )                              //成功清空目录
                    {
...
{
    UINT8   s,i;
    UINT8  xdata buf[100];                                                     //长度可以根据应用自己指定
    UINT16 loc;
//     CfgFsys();
        ...
        if (FoundNewDev)  // 有新的USB设备插入
        {
            ...
            loc = SearchTypeDevice( USB_DEV_CLASS_STORAGE );   // 在ROOT-HUB以及外部HUB各端口上搜索指定类型的设备所在的端口号
                ...
                if ( CH559DiskStatus >= DISK_MOUNTED )                           //U盘准备好
                {
                    ...
                    i = CH559FileCreate( );                                       //新建文件并打开,如果文件已经存在则先删除后再新建 
                    mStopIfError( i );
                    ...
                    i = CH559ByteWrite( );                                        //以字节为单位向文件写入数据,单次读写的长度不能超过MAX_BYTE_IO
                    mStopIfError( i );
                    ...
                    i = CH559FileClose( );                                        //关闭目录,目录不需要关闭,关闭只是为了防止下面误操作
                    mStopIfError( i );
                }
                else
                {
