...
* Date: 2016/06/24
* Description:
 CH559 C language U disk directory creation file,
 The program creates a new /YEAR2004 subdirectory in the root directory of the U disk, and then creates a new file DEMO2004.TXT in this directory and writes data into bytes;
 Finally, create a subdirectory /MONTH05 directory under the /YEAR2004 directory
...
/************************************************* ******************************
* Function Name: mCopyCodeStringToIRAM
* Description: Copy the string in the program space to the internal RAM and return the string length
* Input: UINT8 idata *iDestination, destination string
                   UINT8 code *iSource source string
* Output: None
* Return: UINT8 i returns the number of characters
************************************************** *****************************/
...
* Description: Create a new directory and open it, if the directory already exists, open it directly. The directory name is in mCmdParam.Create.mPathName, which is the same as the file name rule
* Input:
* Output: None
* Return: ERR_SUCCESS = Successfully opened the directory or created the directory successfully,
                   ERR_FOUND_NAME = A file with the same name already exists,
                   ERR_MISS_DIR = The path name is invalid or the parent directory does not exist
************************************************** *****************************/
UINT8 CreateDirectory( void)
{
    ...
    j = 0xFF;
    for (i = 0; i != sizeof( mCmdParam.Create.mPathName ); i ++) //Check the directory path
    {
        ...
        {
            j = i; //Record the parent directory
        }
    }
    i = ERR_SUCCESS;
    if (j == 0 || j == 2 && mCmdParam.Create.mPathName[1] ==':')
    {
        UpDirCluster = 0; //Create a subdirectory under the root directory
    }
    else
    {
        if (j != 0xFF) //For absolute paths, the starting cluster number of the parent directory should be obtained
        {
            mCmdParam.Create.mPathName[ j] = 0;
            i = CH559FileOpen( ); //Open the parent directory
            if (i == ERR_SUCCESS)
            {
                i = ERR_MISS_DIR; //is a file instead of a directory
            }
            else if (i == ERR_OPEN_DIR)
            {
                i = ERR_SUCCESS; //Successfully opened the superior directory
            }
            mCmdParam.Create.mPathName[ j] = PATH_SEPAR_CHAR1; //Restore directory separator
        }
        UpDirCluster = CH559vStartCluster; //Save the starting cluster number of the upper-level directory
    }
    if (i == ERR_SUCCESS) //Successfully obtain the starting cluster number of the upper-level directory
    {
        i = CH559FileOpen( ); //Open this level subdirectory
        if (i == ERR_SUCCESS)
        {
            i = ERR_FOUND_NAME; //It is a file instead of a directory
        }
        else if (i == ERR_OPEN_DIR)
        {
            i = ERR_SUCCESS; //The directory already exists
        }
        else if (i == ERR_MISS_FILE) //The directory does not exist, you can create a new one
        {
            i = CH559FileCreate( ); //Create a directory by creating a file
            if (i == ERR_SUCCESS)
            {
                if (pDISK_FAT_BUF == pDISK_BASE_BUF)
                {
                    memset(pDISK_FAT_BUF,0,CH559vSectorSizeH*256); //If FILE_DATA_BUF is used with DISK_BASE_BUF, the disk buffer must be cleared
                }
                ...
                for (i = 0x40; i != 0; i -) //The reserved unit of the directory, which points to itself and the parent directory respectively
                {
                    *DirXramBuf = *DirConstData;
                    DirXramBuf ++;
                    DirConstData ++;
                }
                *(pDISK_FAT_BUF+0x1A) = ((PUINT8I)&CH559vStartCluster )[3];//Own starting cluster number
                *(pDISK_FAT_BUF+0x1B) = ((PUINT8I)&CH559vStartCluster )[2];
                *(pDISK_FAT_BUF+0x14) = ((PUINT8I)&CH559vStartCluster )[1];
                *(pDISK_FAT_BUF+0x15) = ((PUINT8I)&CH559vStartCluster )[0];
                *(pDISK_FAT_BUF+0x20+0x1A) = ((PUINT8I)&UpDirCluster )[3];//The starting cluster number of the upper-level directory
                *(pDISK_FAT_BUF+0x20+0x1B) = ((PUINT8I)&UpDirCluster )[2];
                *(pDISK_FAT_BUF+0x20+0x14) = ((PUINT8I)&UpDirCluster )[1];
                *(pDISK_FAT_BUF+0x20+0x15) = ((PUINT8I)&UpDirCluster )[0];
// for (count = 0x40; count != CH559vSectorSizeH*256; count ++) {/* Clear the rest of the directory area */
// *DirXramBuf = 0;
// DirXramBuf ++;
//}
                mCmdParam.Write.mSectorCount = 1;
                mCmdParam.Write.mDataBuffer = pDISK_FAT_BUF; //Point to the starting address of the file data buffer
                i = CH559FileWrite( ); //Write data to the file
                if (i == ERR_SUCCESS)
                {
                    ...
                    for (j = 1; j != CH559vSecPerClus; j ++)
                    {
                        ...
                        mCmdParam.Write.mSectorCount = 1;
                        mCmdParam.Write.mDataBuffer = pDISK_FAT_BUF; //Point to the starting address of the file data buffer
                        i = CH559FileWrite( ); //Clear the remaining sectors of the directory
                        if (i != ERR_SUCCESS)
                        {
                            break;
                        }
                    }
                    if (j == CH559vSecPerClus) //Successfully empty the directory
                    {
                        ...
                        mCmdParam.Modify.mFileAttr = 0x10; //Set directory attributes
                        i = CH559FileModify( ); //Modify file information to directory
                    }
                }
            }
        }
...
{
    UINT8 s,i;
    UINT8 xdata buf[100]; //The length can be specified according to the application
    UINT16 loc;
// CfgFsys();
...
SelectHubPort(i); // Select the ROOT-HUB port specified by the operation, set the current USB speed and the USB address of the operated device
                ...
                if (CH559DiskStatus >= DISK_MOUNTED) //U disk is ready
                {
                    ...
                    strcpy( mCmdParam.Create.mPathName, "/YEAR2004" ); //Directory name, the directory is built in the root directory
                    i = CreateDirectory( ); //Create or open a directory
                    mStopIfError( i );
                    /* The directory is created or opened successfully, and a demo file is created in this subdirectory below */
                    printf( "Create New File /YEAR2004/DEMO2004.TXT \n" );
                    strcpy( mCmdParam.Create.mPathName, "/YEAR2004/DEMO2004.TXT" );//file name
                    i = CH559FileCreate( ); //Create a new file and open it, if the file already exists, delete it and then create it
                    mStopIfError( i );
                    printf( "Write some data to file DEMO2004.TXT \n" );
                    i = sprintf( buf, "demo file\xd\xa" );
                    ...
                    i = CH559ByteWrite( ); //Write data to the file in bytes, and the length of a single read and write cannot exceed MAX_BYTE_IO
                    mStopIfError( i );
                    printf( "Close file DEMO2004.TXT \n" );
                    mCmdParam.Close.mUpdateLen = 1; //Automatically calculate the file length and write the file in bytes. It is recommended that the library close the file to automatically update the file length
                    i = CH559FileClose( );
                    mStopIfError( i );
                    /* Create a new second-level subdirectory below, the method is exactly the same as the previous first-level subdirectory */
                    printf( "Create Level 2 Directory /YEAR2004/MONTH05 \n" );
                    strcpy( mCmdParam.Create.mPathName, "/YEAR2004/MONTH05" ); //The directory name, the directory is built in the YEAR2004 subdirectory, and the YEAR2004 directory must exist in advance
                    i = CreateDirectory( ); //Create or open a directory
                    mStopIfError( i );
                    printf( "Close\n" );
                    mCmdParam.Close.mUpdateLen = 0; //No need to automatically update file length for directories
                    i = CH559FileClose( ); //Close the directory, the directory does not need to be closed, the closing is just to prevent the following misoperation
                    mStopIfError( i );
                }
                else
                {
