...
*****************************************
*/
/* CH559 U盘主机文件系统接口, 支持: FAT12/FAT16/FAT32 */

//#define DISK_BASE_BUF_LEN		512	/* 默认的磁盘数据缓冲区大小为512字节(可以选择为2048甚至4096以支持某些大扇区的U盘),为0则禁止在本文件中定义缓冲区并由应用程序在pDISK_BASE_BUF中指定 */
/* 如果需要复用磁盘数据缓冲区以节约RAM,那么可将DISK_BASE_BUF_LEN定义为0以禁止在本文件中定义缓冲区,而由应用程序在调用CH559LibInit之前将与其它程序合用的缓冲区起始地址置入pDISK_BASE_BUF变量 */
...
//    CH559vSectorSizeH=512>>8;  // 设置实际的扇区大小,必须是512的倍数,该值是磁盘的扇区大小的高字节,低字节总是0x00,数值2代替0x200
//    CH559vSectorSizeB=9;   // 设置实际的扇区大小的位移数,512则对应9,1024对应10,2048对应11
//    CH559DiskStatus=DISK_MOUNTED;  // 强制块设备连接成功(只差分析文件系统)
//}

UINT8	CH559ReadSector( UINT8 SectCount, PUINT8X DataBuf )  /* 从磁盘读取多个扇区的数据到缓冲区中 */
{
	UINT8	retry;
//	if ( use_external_interface ) return( extReadSector( CH559vLbaCurrent, SectCount, DataBuf ) );  /* 外部接口 */
	for( retry = 0; retry < 3; retry ++ ) {  /* 错误重试 */
		...
		CH559BulkOnlyCmd( DataBuf );  /* 执行基于BulkOnly协议的命令 */
	...
	return( CH559IntStatus = ERR_USB_DISK_ERR );  /* 磁盘操作错误 */
}

#ifdef	EN_DISK_WRITE
UINT8	CH559WriteSector( UINT8 SectCount, PUINT8X DataBuf )  /* 将缓冲区中的多个扇区的数据块写入磁盘 */
{
	UINT8	retry;
//	if ( use_external_interface ) return( extWriteSector( CH559vLbaCurrent, SectCount, DataBuf ) );  /* 外部接口 */
	for( retry = 0; retry < 3; retry ++ ) {  /* 错误重试 */
		pCBW -> mCBW_DataLen0 = 0;  /* 数据传输长度 */
		pCBW -> mCBW_DataLen1 = SectCount << CH559vSectorSizeB - 8;
		pCBW -> mCBW_DataLen2 = 0;
...
{
	USB_DEV_AD &= 0x7F;
	if ( USB_DEV_AD == USB_DEVICE_ADDR || USB_DEV_AD == USB_DEVICE_ADDR + 1 ) {  /* 内置Root-HUB下的USB设备 */
		if ( ( USB_DEV_AD == USB_DEVICE_ADDR ? UHUB0_CTRL : UHUB1_CTRL ) & bUH_PORT_EN ) {  /* 内置Root-HUB下的USB设备存在且未插拔 */
			return( ERR_SUCCESS );  /* USB设备已经连接且未插拔 */
		}
		else if ( USB_HUB_ST & (UINT8)( USB_DEV_AD == USB_DEVICE_ADDR ? bUHS_H0_ATTACH : bUHS_H1_ATTACH ) ) {  /* 内置Root-HUB下的USB设备存在 */
mDiskConnect:
			CH559DiskStatus = DISK_CONNECT;  /* 曾经断开过 */
...
mDiskDisconn:
	...
	else if ( USB_DEV_AD > 0x10 && USB_DEV_AD <= 0x14 || USB_DEV_AD > 0x20 && USB_DEV_AD <= 0x24 ) {  /* 外部HUB的端口下的USB设备 */
		if ( ( USB_DEV_AD & 0x20 ? UHUB1_CTRL : UHUB0_CTRL ) & bUH_PORT_EN ) {  /* 内置Root-HUB下的外部HUB存在且未插拔 */
			TxBuffer[ MAX_PACKET_SIZE - 1 ] = USB_DEV_AD;  /* 备份 */
			USB_DEV_AD = USB_DEVICE_ADDR - 1 + ( USB_DEV_AD >> 4 );  /* 设置USB主机端的USB地址指向HUB */
			CH559IntStatus = HubGetPortStatus( TxBuffer[ MAX_PACKET_SIZE - 1 ] & 0x0F );  /* 查询HUB端口状态,返回在TxBuffer中 */
			if ( CH559IntStatus == ERR_SUCCESS ) {
				if ( TxBuffer[2] & (1<<(HUB_C_PORT_CONNECTION-0x10)) ) {  /* 检测到HUB端口上的插拔事件 */
					CH559DiskStatus = DISK_DISCONNECT;  /* 假定为HUB端口上的USB设备断开 */
...
//					return( ERR_USB_DISCON );
					...
					CH559DiskStatus = DISK_CONNECT;  /* HUB操作失败 */
					return( CH559IntStatus );
				}
			}
		}
		else if ( USB_HUB_ST & (UINT8)( USB_DEV_AD & 0x20 ? bUHS_H1_ATTACH : bUHS_H0_ATTACH ) ) {  /* 内置Root-HUB下的USB设备存在,外部HUB或USB设备已经连接或者断开后重新连接 */
//			CH559DiskStatus = DISK_CONNECT;  /* 曾经断开过 */
//			return( ERR_SUCCESS );  /* 外部HUB或USB设备已经连接或者断开后重新连接 */
			goto mDiskConnect;
		}
...
{
/* 如果指定枚举序号CH559vFileSize为0xFFFFFFFF后调用FileOpen，那么每搜索到一个文件FileOpen都会调用本回调程序，
   回调程序xFileNameEnumer返回后，FileOpen递减CH559vFileSize并继续枚举直到搜索不到文件或者目录。建议做法是，
   在调用FileOpen之前定义一个全局变量为0，当FileOpen回调本程序后，本程序由CH559vFdtOffset得到结构FAT_DIR_INFO，
   ...
   如果在本回调程序xFileNameEnumer中将CH559vFileSize置为1，那么可以通知FileOpen提前结束搜索。以下是回调程序例子 */
#if		0
	UINT8			i;
	UINT16			FileCount;
	PX_FAT_DIR_INFO	pFileDir;
	PUINT8			NameBuf;
	pFileDir = (PX_FAT_DIR_INFO)( pDISK_BASE_BUF + CH559vFdtOffset );  /* 当前FDT的起始地址 */
	FileCount = (UINT16)( 0xFFFFFFFF - CH559vFileSize );  /* 当前文件名的枚举序号,CH559vFileSize初值是0xFFFFFFFF,找到文件名后递减 */
...
UINT8	CH559LibInit( void )  /* 初始化CH559程序库,操作成功返回0 */
{
	if ( CH559GetVer( ) < CH559_LIB_VER ) return( 0xFF );  /* 获取当前子程序库的版本号,版本太低则返回错误 */
#if		DISK_BASE_BUF_LEN > 0
...
#endif
	CH559DiskStatus = DISK_UNKNOWN;  /* 未知状态 */
	CH559vSectorSizeB = 9;  /* 默认的物理磁盘的扇区是512B */
	CH559vSectorSizeH = 512 >> 8;  // 默认的物理磁盘的扇区是512B,该值是磁盘的扇区大小的高字节,低字节总是0x00,数值2代替0x200
	CH559vStartLba = 0;  /* 默认为自动分析FDD和HDD */
	return( ERR_SUCCESS );
}
